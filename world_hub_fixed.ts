...
function spawnNPC(npcId: string, x: number, y: number, dialogId: string) {
  // DECISION: Use SpriteKind.Enemy as workaround for KIND_NPC initialization issues
  // KIND_NPC may fail to cast in MakeCode's runtime, so using built-in kind instead
...
function spawnDoor(dungeonId: string, x: number, y: number) {
  // DECISION: Use SpriteKind.Food as workaround for KIND_DOOR initialization issues
  // KIND_DOOR may fail to cast in MakeCode's runtime, so using built-in kind instead
